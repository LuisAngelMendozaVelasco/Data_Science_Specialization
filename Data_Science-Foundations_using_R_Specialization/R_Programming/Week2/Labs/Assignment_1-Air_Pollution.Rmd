...
```{bash}
unzip -q ./data/specdata.zip -d ./data/
```
...
complete <- function(directory, id=1:332) {
    ...
    for(i in id) {
        ...
        sulfate = df[["sulfate"]]
        nitrate = df[["nitrate"]]
        nobs = append(nobs, sum(!is.na(sulfate) & !is.na(nitrate)))
    }
...
corr <- function(directory, threshold=0) {
    ...
    ids = complete_df[["id"]][complete_df[["nobs"]] > threshold]
    correlation = c()
    for(i in ids) {
        if(i == 0){
            correlation
            ...
            sulfate = df[["sulfate"]]
            nitrate = df[["nitrate"]]
            correlation = append(correlation, cor(sulfate, nitrate, use="complete.obs"))
        }
    }
    correlation
}
