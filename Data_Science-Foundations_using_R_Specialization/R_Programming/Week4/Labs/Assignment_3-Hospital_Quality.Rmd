...
```{r}
outcome <- read.csv("./data/outcome-of-care-measures.csv.gz", colClasses="character")
head(outcome)
```
...
best <- function(state, outcome) {
    ## Read outcome data
    data <- read.csv("./data/outcome-of-care-measures.csv.gz", colClasses="character", check.names=FALSE)
    ...
    ## Return hospital name in that state with lowest 30-day death
    ## rate
    colnames(data) = tolower(gsub('^Hospital 30-Day Death \\(Mortality\\) Rates from ', '', colnames(data)))
    outcome = tolower(outcome)
    data_state = data[data[["state"]]==state,]
    data_state = data_state[, c("hospital name", outcome)]
    data_state[[outcome]] = as.numeric(data_state[[outcome]])
    data_state = data_state[order(data_state[[outcome]], data_state[["hospital name"]]),]
    data_state[1, "hospital name"]
}
```
...
rankhospital <- function(state, outcome, num="best") {
    ...
    if(num=="best") {
        ...
        tail(data_state, 1)[["hospital name"]]
    }
    else {
        if(num <= nrow(data_state)) {
            data_state[num, "hospital name"] 
        }
        else {
            NA
        }
...
rankall <- function(outcome, num="best") {
    ...
    for (state in states) {
        ...
        data_state = data_state[order(data_state[[outcome]], data_state[["hospital name"]]),]
        if(num=="best") {
            row = c(data_state[1, "hospital name"], state)
            df = rbind(df, row)
        }
        else if(num=="worst") {
            row = c(tail(data_state, 1)[["hospital name"]], state)
            df = rbind(df, row)
        }
        else {
            if(num <= nrow(data_state)) {
                row = c(data_state[num, "hospital name"], state)
                df = rbind(df, row)
            }
            else {
                row = c(NA, state)
